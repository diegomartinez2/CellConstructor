...
#            print "New: %e | Old: %e" % (new_w[i], w[i])
    ...
    """
        ...
        size = int(size)
        rand = np.random.normal(size = (size, n_modes))#Diego's note: Here random numbers are created following a Gaussian distribution aka Normal distribution because the density matrix of SSCHA is Gaussian by definition. 
